...
#specify polygon shapefile vector for 1st day
...
iface.addVectorLayer(ffgs_filepath, input_datetime + '_hours_0-24','ogr')		#add shapefile to the map and directory/list of layers

# specify raster filename
rasterFilePath24 = "wrf_" + input_datetime +"_f24.grib2"		# first 24-hour precip interval filepath

# usage - QgsZonalStatistics (QgsVectorLayer: polygonLayer, const QString: rasterFile, const QString attributePrefix="desired_prefix", int rasterBand=1)
...
zoneStat = QgsZonalStatistics (ffgs, rasterFilePath24, '0-24_hours', 1, QgsZonalStatistics.Mean)
zoneStat.calculateStatistics()

print('0-24 hour mean was created')

#specify polygon shapefile vector for 2nd day
ffgs_filepath = 'C:/Users/User/Desktop/Capstone/Shapefiles/ffgs_basin_view2.shp'		#edit path to match location of shapefile on user's device
ffgs = QgsVectorLayer('C:/Users/User/Desktop/Capstone/Shapefiles/ffgs_basin_view2.shp','FFGS','ogr')		#Stores the shapefile layer as a variable
iface.addVectorLayer(ffgs_filepath, input_datetime + '_hours_24-48','ogr')

rasterFilePath48 = tiff_path_48		# Second 24-hour precip interval filepath

zoneStat = QgsZonalStatistics (ffgs, rasterFilePath48, '24-48_hours', 1, QgsZonalStatistics.Mean)
zoneStat.calculateStatistics()
